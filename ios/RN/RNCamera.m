...
//        [self setupMovieFileCapture];
//        NSLog(@"barcode-on setupMovieFileCapture");
...
{
    NSLog(@"barcode-on createBarcodeDetectorMlKit");
...
{
    NSLog(@"barcode-on setupOrDisableBarcodeDetector");
    if (self.canDetectBarcodes){
        NSLog(@"barcode-on YES setupOrDisableBarcodeDetector");
        AVCaptureSessionPreset preset = [self getDefaultPresetVideo];

        self.session.sessionPreset = preset;
        if (!self.videoDataOutput) {
            NSLog(@"barcode-on YES videoDataOutput");
            self.videoDataOutput = [[AVCaptureVideoDataOutput alloc] init];
            if (![self.session canAddOutput:_videoDataOutput]) {
                NSLog(@"barcode-on Failed to setup video data output");
                [self stopBarcodeDetection];
                return;
            }
        ...
        NSLog(@"barcode-on NO setupOrDisableBarcodeDetector");
        [self stopBarcodeDetection];
    }
}
...
{
    NSLog(@"barcode-on stopBarcodeDetection");
...
//    NSLog(@"barcode-on capture");
    ...
    BOOL canSubmitForBarcodeDetection = timePassedSinceSubmittingForBarcode > 0.25 && _finishedDetectingBarcodes && self.canDetectBarcodes;
    if (canSubmitForBarcodeDetection) {
        NSLog(@"barcode-on canSubmitForBarcodeDetection TRUE");
        ...
        if (canSubmitForBarcodeDetection) {
            ...
            if (imageBuffer) {
                NSLog(@"barcode-on YES imageBuffer");
                ...
                NSLog(@"barcode-on NO imageBuffer");
            }
        }
    }
}
...
- (void)scanBarcodesOnDeviceInImage:(MLKVisionImage *)image
  ...
  dispatch_sync(dispatch_get_main_queue(), ^{
    ...
    if (error != nil) {
      NSLog(@"barcode-on Failed to scan barcodes with error: %@", error.localizedDescription);
      self.finishedDetectingBarcodes = true;
      return;
    }
    if (barcodes.count == 0) {
      NSLog(@"barcode-on On-Device barcode scanner returned no results.");
      self.finishedDetectingBarcodes = true;
      return;
    }
      ...
      for (MLKBarcode *barcode in barcodes) {
          ...
          NSString *rawValue = [barcode.rawValue stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
          NSString *displayValue = [barcode.displayValue stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceCharacterSet]];
...
//      [description appendString:barcodes[0].rawValue];
//      NSLog([NSString stringWithFormat:@"barcode-on READED BARCODE my dictionary is %@", description]);
      
      NSDictionary *eventBarcode = @{@"type" : @"barcode", @"barcodes" : result};
